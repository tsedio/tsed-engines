...
describe("VueEngine", () => {
  ...
  it("should render the given content (by string - no cache)", async () => {
    const {render, $compile} = getEngineFixture({token: VueEngine});
    await render();
  ...
  it("should render the given content (by string - with cache)", async () => {
    const {render, $compile} = getEngineFixture({token: VueEngine, cache: true});
    await render();
  ...
  it("should render the given content (by file - no cache)", async () => {
    const {renderFile, $compileFile} = getEngineFixture({token: VueEngine});
    await renderFile();
  ...
  it("should render the given content (by file - with cache)", async () => {
    const {renderFile, $compileFile} = getEngineFixture({token: VueEngine, cache: true});
