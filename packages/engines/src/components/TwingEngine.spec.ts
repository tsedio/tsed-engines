...
describe("TwingEngine", () => {
  it("should render the given content (by string - no cache)", async () => {
    const {render, $compile} = getEngineFixture({token: TwingEngine});
    await render();
  ...
  it("should render the given content (by string - with cache)", async () => {
    const {render, $compile} = getEngineFixture({token: TwingEngine, cache: true});
    await render();
  ...
  it("should render the given content (by file - no cache)", async () => {
    const {renderFile, $compileFile} = getEngineFixture({token: TwingEngine});
    await renderFile();
  ...
  it("should render the given content (by file - with cache)", async () => {
    const {renderFile, $compileFile, path} = getEngineFixture({token: TwingEngine, cache: true});
