...
describe("VelocityEngine", () => {
  it("should render the given content (by string - no cache)", async () => {
    const {render, locals, $compile} = getEngineFixture({token: "velocityjs"});
    await render();
  ...
  it("should render the given content (by string - with cache)", async () => {
    const {render, locals, $compile} = getEngineFixture({token: VelocityEngine, cache: true});
    await render();
  ...
  it("should render the given content (by file - no cache)", async () => {
    const {renderFile, locals, $compile, path, template} = getEngineFixture({token: VelocityEngine});
    await renderFile();
  ...
  it("should render the given content (by file - with cache)", async () => {
    const {renderFile, locals, $compile, path, template} = getEngineFixture({token: VelocityEngine, cache: true});
