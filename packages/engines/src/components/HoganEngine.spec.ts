...
describe("HoganEngine", () => {
  it("should render the given content (by string - no cache)", async () => {
    const {render, locals, $compile, template} = getEngineFixture({token: HoganEngine});
    await render();
  ...
  it("should render the given content (by string - with cache)", async () => {
    const {render, locals, $compile, template} = getEngineFixture({token: HoganEngine, cache: true});
    await render();
  ...
  it("should render the given content (by file - no cache)", async () => {
    const {renderFile, locals, $compile, path, template} = getEngineFixture({token: HoganEngine});
    await renderFile();
  ...
  it("should render the given content (by file - with cache)", async () => {
    const {renderFile, locals, $compile, path, template} = getEngineFixture({token: HoganEngine, cache: true});
  ...
  it("should support partials", async () => {
    const {renderFile} = getEngineFixture({token: HoganEngine, templateName: "partials"});
    const locals = {
      partials: {
        partial: "user"
      }
  ...
  it("should support absolute path partial", async () => {
    const {renderFile, rootDir, name} = getEngineFixture({token: HoganEngine, templateName: "partials"});
  ...
  it("should support relative path partial", async () => {
    const {renderFile, name} = getEngineFixture({token: HoganEngine, templateName: "partials"});
