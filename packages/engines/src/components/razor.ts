...
const from = () =>
  (path: string, options: any, cb: EngineCallback): Promise<any> => {
    return promisify(cb, function (cb) {
      const engine = getEngine();
      try {
        const tmpl = cache(options) || cache(options, (locals: any) => {
...
export default registerEngine("razor", (str, options, cb) => {
  return promisify(cb, function (cb) {
    const engine = getEngine();
