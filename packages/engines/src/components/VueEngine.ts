...
export class VueEngine extends Engine {
  ...
  constructor(name: string, options: ViewEngineOptions) {
    ...
    requireEngine("vue-pronto/lib/index", "pronto");
  }

  get pronto() {
    return requireEngine("pronto");
  }

  get vueify() {
    return requireEngine("vueify");
  }

  get vueServerRenderer() {
    return requireEngine("vue-server-renderer");
  }
