...
describe("SquirrellyEngine", () => {
  it("should render the given content (by string - no cache)", async () => {
    const {render, locals, $compile, template} = getEngineFixture({token: SquirrellyEngine});
    await render();
  ...
  it("should render the given content (by string - with cache)", async () => {
    const {render, locals, $compile, template} = getEngineFixture({token: SquirrellyEngine, cache: true});
    await render();
  ...
  it("should render the given content (by file - no cache)", async () => {
    const {renderFile, locals, $compile, path, template} = getEngineFixture({token: SquirrellyEngine});
    await renderFile();
  ...
  it("should render the given content (by file - with cache)", async () => {
    const {renderFile, locals, $compile, path, template} = getEngineFixture({token: SquirrellyEngine, cache: true});
  ...
  it("should support helpers", async () => {
    ...
    const {render} = getEngineFixture({token: SquirrellyEngine, templateName: "helpers"});
  ...
  it("should support partials", async () => {
    const {renderFile} = getEngineFixture({token: SquirrellyEngine, templateName: "partials"});
    const locals = {
      partials: {
        partial: "user"
      }
  ...
  it("should support absolute path partial", async () => {
    const {renderFile, rootDir, name} = getEngineFixture({token: SquirrellyEngine, templateName: "partials"});
  ...
  it("should support relative path partial", async () => {
    const {renderFile, name} = getEngineFixture({token: SquirrellyEngine, templateName: "partials"});
