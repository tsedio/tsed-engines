import {cache, extend, read, readPartials, requireEngine} from "../utils/cache";
...
export class Engine {
  ...
  constructor(readonly name: string, options: ViewEngineOptions) {
    const reqs = ([] as string[]).concat(options.requires || [name]);

    reqs.forEach((req: string) => {
      try {
        requireEngine(req, name);
        this.driverName = req;
      } catch (er) {}
    });
  }
  ...
  get engine(): any {
    return requireEngine(this.name);
  }
