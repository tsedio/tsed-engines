...
describe("RactiveEngine", () => {
  it("should render the given content (by string - no cache)", async () => {
    const {render, locals, $compile, template} = getEngineFixture({token: RactiveEngine});
    await render();
  ...
  it("should render the given content (by string - with cache)", async () => {
    const {render, $compile, template} = getEngineFixture({token: RactiveEngine, cache: true});
    await render();
  ...
  it("should render the given content (by file - no cache)", async () => {
    const {renderFile, $compileFile} = getEngineFixture({token: RactiveEngine});
    await renderFile();
  ...
  it("should render the given content (by file - with cache)", async () => {
    const {renderFile, $compileFile} = getEngineFixture({token: RactiveEngine, cache: true});
