...
describe("TwigEngine", () => {
  it("should render the given content (by string - no cache)", async () => {
    const {render, $compile, template} = getEngineFixture({token: TwigEngine});
    await render();
  ...
  it("should render the given content (by string - with cache)", async () => {
    const {render, $compile} = getEngineFixture({token: TwigEngine, cache: true});
    await render();
  ...
  it("should render the given content (by file - no cache)", async () => {
    const {renderFile, $compileFile, path} = getEngineFixture({token: TwigEngine});
    await renderFile();
  ...
  it("should render the given content (by file - with cache)", async () => {
    const {renderFile, $compileFile} = getEngineFixture({token: TwigEngine, cache: true});
